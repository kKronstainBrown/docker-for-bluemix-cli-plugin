...
In the terminal, you are notified when updates to the `ibmcloud15` CLI and plug-ins are available. Be sure to keep your CLI up-to-date so that you can use all the available commands and flags.
...
Create service credentials of an {{site.data.keyword.Bluemix_notm}} service and store these credentials in a Kubernetes secret in your cluster. To view available {{site.data.keyword.Bluemix_notm}} services from the {{site.data.keyword.Bluemix_notm}} catalog, run `ibmcloud15 service offerings`. **Note**: You can only add {{site.data.keyword.Bluemix_notm}} services that support service keys.
{: shortdesc}
...
<dd>The name of the {{site.data.keyword.Bluemix_notm}} service instance that you want to bind. To find the name, run <code>ibmcloud15 service list</code> for Cloud Foundry services, and <code>ibmcloud15 resource service-instances</code> for IAM-enabled services. This value is required. </dd>
...
To list available roles for the service, run `ibmcloud15 iam roles --service <service_name>`. The service name is the name of the service in the catalog which you can get by running `ibmcloud15 catalog search`.  </dd>
...
<dd>Your {{site.data.keyword.keymanagementserviceshort}} instance GUID. To get the instance GUID, run <code>ibmcloud15 resource service-instance SERVICE_INSTANCE_NAME --id</code> and copy the second value (not the full CRN).</dd>
